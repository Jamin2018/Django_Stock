...
while True:
    code_list = pd.read_csv(data_dir + '/code_list.csv', dtype={'symbol':str})
    time.sleep(3)
    for code in monitoring_code:
        ...
        name = code_list[code_list['symbol'] == code]['name'].values[0]
        if k >= d:
            print(
                """\033[31m%s  %s %s | K(%.1f)  D(%.1f) J(%.1f) | 昨收:%s  当前:%s""" % (df['time'].values[0], name, code, k, d, j, df_y[:1]['close'].values[0], df['close'].values[0])
            )
        else:
            print(
                """\033[30m%s  %s %s | K(%.1f)  D(%.1f) J(%.1f) | 昨收:%s  当前:%s""" % (df['time'].values[0], name, code, k, d, j, df_y[:1]['close'].values[0], df['close'].values[0])
            )
    print('\033[30m---------------------------------------------------------------------------------------------------')
