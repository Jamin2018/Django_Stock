...
class TickData:
    ...
    def __init__(self, symbol, timestamp, last_price=0, total_vol=0):
        self.symbol = symbol
...
class MarketDataSource:
    ...
    def get_ts_data(self, ticker, start, end):
        self.data = ts.get_hist_data(ticker,start,end)

    def start_market_simulation(self):
        self.get_ts_data(self.ticker, self.start, self.end)
...
class Position:
    ...
    def event_fill(self, is_buy, qty, price):
        if is_buy:
            self.buys += qty
        else:
            self.sells += qty
...
class MeanRevertingStrategy(Strategy):
    ...
    """
    def __init__(self, symbol, look_back_intervals=20, buy_threshold=-1.5, sell_threshold=1.5):
...
class BackTester:
    ...
    def update_filled_positions(self, symbol, qty, is_buy, price, timestamp):
        ...
        print(self.get_trade_date(), "Filled:", "Buy" if is_buy else "Sell", qty, symbol, "at", price)
    ...
    def event_handler_order(self, order):
        self.unfilled_orders.append(order)
        print(self.get_trade_date(), "Received order:", "Buy" if order.is_buy else "Sell", order.qty, order.symbol)
    ...
    def is_order_unmatched(self, order, prices):
        ...
        if order.is_market_order and timestamp > order.timestamp:
            ...
            self.update_filled_positions(symbol, order.qty, order.is_buy, open_price, timestamp)
    ...
    def event_handler_tick(self, prices):
    ...
    def start_back_test(self):
        ...
        self.strategy.event_send_order = self.event_handler_order
        mds = MarketDataSource()
        mds.event_tick = self.event_handler_tick
...
if __name__ == '__main__':
    back_tester = BackTester("600000", '2018-01-01', '2019-01-01')
