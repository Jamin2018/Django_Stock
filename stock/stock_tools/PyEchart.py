...
class StockPyEchart(object):
    def __init__(self, config_dict):
        ...
        self.SEE = StockEigenvalueExtraction(config_dict)
    ...
    def draw_kline_v1(self, df, filename):
        ...
        df = df.dropna(axis=0, how='any', subset=[self.stock_csv_columns_dict['收盘价'],])
        df = self.SEE.kdj(df)
        df = self.SEE.kdj_macd(df)
        df = self.SEE.macd(df)
        df = self.SEE.obv(df)
        df = self.SEE.ma(df, 5)
        df = self.SEE.ma(df, 20)
        df = self.SEE.ma(df, 30)

        data = []
        for i, row in df.iterrows():
            data.append([row[self.stock_csv_columns_dict['开盘价']], row[self.stock_csv_columns_dict['收盘价']],
                         row[self.stock_csv_columns_dict['最低价']], row[self.stock_csv_columns_dict['最高价']]])
        echarts_data = {
            'data': data,
            'date': list(df.index),
            'volume': list(df[self.stock_csv_columns_dict['成交量']]),
            'ma5': list(df['ma5']),
            'ma20': list(df['ma20']),
            'ma30': list(df['ma30']),
            'macd': list(df['macd']),
    ...
    def draw_charts(self, df, filename=None):
        ...
        charts = self.draw_kline_v1(df, filename)
        return charts


def get_current_df(code, ktype='d'):
    """
    FIXME （字段名变了）将昨日的d线数据和当前最新数据结合，为了得出当前的K图
    """
